...
## Current Phase
Phase 10 COMPLETE: State Management & Error Handling — replaced every screen's ad-hoc `useEffect`/`fetch` with TanStack Query hooks (`useGraph`, `useRisk`, `useBrief`, `useIntegrity`, `useCasesQuery`/`useCreateCase`, `useEvidence`) backed by one `QueryClient` (`frontend/src/lib/queryClient.ts`) with a shared cache-key factory (`lib/queryKeys.ts`). `useEvidence`'s upload mutation invalidates `graph`/`risk`/`integrity`/`brief` for the case on success, so a new file dropped on Intake propagates to Correlation, Risk Desk, and Brief without a manual page refresh — verified against a live backend with the Phase 2 fixture set uploaded across all four source types in sequence. Added a global error-toast pattern (`sonner`, wrapped as `AppToaster` + `lib/toast.ts`, styled cream/khaki surface with terracotta errors) driven by the QueryClient's own `onError`, plus `AppErrorBoundary` (mounted once in `main.tsx`) for the render-time-exception class of failure toasts can't catch — neither path ever surfaces a raw stack trace to the investigator. `CaseContext` now composes `useCasesQuery`/`useCreateCase` with a single `activeCaseId` `useState`, keeping React Context (not Zustand) as the one piece of shared UI state — see `docs/architecture.md#state-management` for the full reasoning. `IntakeScreen`'s existing pending→uploading→processed/failed optimistic chip state (Phase 9) is unchanged in behavior, now wired through `useEvidence.mutateAsync`. Frontend build and lint pass with 0 warnings; backend untouched, all 97 tests still apply.
